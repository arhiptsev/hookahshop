...
export const ProductsView = ({ products, cartEnable }) => {
  const [addToCartMutate] = useMutation(ADD_TO_CART_QUERY,
    {
      ...
      onCompleted: () => fetchNotifyHandler(
  ...
  const addToCart = (id: number) => addToCartMutate({
    variables: { id },
        ...
        {products.map((product, index) => (
          <Card style={{ width: '18rem' }} key={index}>
            <Card.Body>
              ...
              {cartEnable && (<Button variant="primary" onClick={() => addToCart(product.id)}>В корзину</Button>)}
            </Card.Body>
          </Card>
        ))}
      </ProductGrid>
